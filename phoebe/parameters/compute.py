...
def phoebe(**kwargs):
    ...
    * (<phoebe.parameters.ParameterSet>): ParameterSet of all newly created
    ...
    params += [ChoiceParameter(visible_if='hierarchy.is_meshable:true', copy_for={'kind': ['star', 'envelope', 'disk'], 'component': '*'},
    ...
    params += [IntParameter(visible_if='mesh_method:marching,hierarchy.is_meshable:true', copy_for={'kind': ['star', 'envelope', 'disk'], 'component': '*'}, component='_default', qualifier='ntriangles', value=kwargs.get('ntriangles', 1500), limits=(100,None), default_unit=u.dimensionless_unscaled, description='Requested number of triangles (won\'t be exact).')]
    ...
    params += [ChoiceParameter(copy_for = {'kind': ['disk'], 'component': '*'}, component='_default', qualifier='atm', value=kwargs.get('atm', 'blackbody'), choices=_atm_choices, description='Atmosphere table')]
    params += [ChoiceParameter(visible_if='atm:!blackbody,atm:!extern_planckint,atm:!extern_atmx', copy_for = {'kind': ['star', 'disk'], 'component': '*'}, component='_default', qualifier='atm_extrapolation_method', value=kwargs.get('atm_extrapolation_method', 'linear'), choices=['none', 'nearest', 'linear'], description='Method of extrapolating intensities outside of the atmosphere grid')]
    params += [ChoiceParameter(visible_if='atm:!blackbody,atm:!extern_planckint,atm:!extern_atmx', copy_for = {'kind': ['star', 'disk'], 'component': '*'}, component='_default', qualifier='ld_extrapolation_method', value=kwargs.get('ld_extrapolation_method', 'nearest'), choices=['none', 'nearest', 'linear'], description='Method of extrapolating limb-darkening intensities outside of the atmosphere grid')]
    params += [ChoiceParameter(visible_if='atm:!blackbody,atm:!extern_planckint,atm:!extern_atmx,atm_extrapolation_method:!none,ld_extrapolation_method:!none', copy_for = {'kind': ['star', 'disk'], 'component': '*'}, component='_default', qualifier='blending_method', value=kwargs.get('blending_method', 'blackbody'), choices=['none', 'blackbody'], description='Method to use for blending model atmosphere and blackbody intensities off the atmosphere grid')]
    params += [FloatParameter(visible_if='atm:!blackbody,atm:!extern_planckint,atm:!extern_atmx,atm_extrapolation_method:!none', copy_for = {'kind': ['star', 'disk'], 'component': '*'}, component='_default', qualifier='extrapolation_max_frac', value=kwargs.get('extrapolation_max_frac', 0.25), limits=(0,1), default_unit=u.dimensionless_unscaled, description='Fraction of the number of surface elements allowed to be outside of the atmosphere grid before an error is raised.  Set to 1 to allow all surface elements to be outside of the atmosphere grid without warning.')]
