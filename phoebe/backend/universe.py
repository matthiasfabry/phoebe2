...
class Disk(Body):
    ...
    A Disk has no orbital dynamics of its own -- it is a child of a parent
    Star and rides along with it: position, velocity, and orientation are
    read from the same index (ind_self/ind_self_vel) that the dynamics
    ...
    @property
    def instantaneous_maxr(self):
        return self.outer_radius * self._scale
    
    @property
    def ind_self(self):
        """
        No dynamics of our own -- always read position/euler angles at the
        same index as our parent star.
        """
        return self.parent.ind_self

    @property
    def ind_self_vel(self):
        return self.parent.ind_self_vel
    ...
    def _build_mesh(self, *args, **kwargs):
        """
        Build the (static, body-centric) disk mesh.  The disk shape never
        changes in time, so this only ever needs to run once -- see
        update_position, which caches the result via save_as_standard_mesh.
        """
        ntriangles = kwargs.get('ntriangles', 1000)
        new_mesh_dict = libphoebe.disk_mesh(self.inner_radius, self.outer_radius,
                                            self.height, ntriangles, 
                                            vertices=True,
                                            triangles=True,
                                            centers=True,
                                            vnormals=True,
    ...
    def update_position(self, time,
                        ...
                        **kwargs):
        """
        Update the position of the disk to match its parent star's current
        position/velocity/orientation in the system.
        """
        logger.debug("{}.update_position ignore_effects={}".format(self.component, ignore_effects))
        self.reset_time(time, ethetas[self.ind_self], elongans[self.ind_self], eincls[self.ind_self])

        # -- read position/velocity/orientation from the parent's index --
        pos = (_value(xs[self.ind_self]), _value(ys[self.ind_self]), _value(zs[self.ind_self]))
        vel = (_value(vxs[self.ind_self_vel]), _value(vys[self.ind_self_vel]), _value(vzs[self.ind_self_vel]))
        euler = (_value(ethetas[self.ind_self]), _value(elongans[self.ind_self]), _value(eincls[self.ind_self]))
        euler_vel = (_value(ethetas[self.ind_self_vel]), _value(elongans[self.ind_self_vel]), _value(eincls[self.ind_self_vel]))

        # the disk geometry is static -- build it once and reuse forever
        if not self.has_standard_mesh():
            logger.debug("{}.update_position: building disk mesh".format(self.component))
            new_mesh_dict = self._build_mesh(**kwargs)
